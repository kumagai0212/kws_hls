...
VPP     := /tools/Xilinx/2025.1/Vitis/bin/v++
...
prog:
	mkdir -p build
	$(GCC) -Os -march=rv32im -mabi=ilp32 -nostartfiles -Iapp -Tapp/link.ld -o build/main.elf app/crt0.s app/*.c *.c -lm
	make initf
	cat build/memi.bin build/memd.bin > build/uart_mem.bin
...
vpp:
	...
	mkdir -p cfu
	for f in vitis/hls/impl/verilog/*.v; do \
		echo '`default_nettype wire' | cat - "$$f" > cfu/$$(basename "$$f"); \
	done
	-cp vitis/hls/impl/verilog/*.tcl cfu/. 2>/dev/null || true
